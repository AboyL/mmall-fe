...
var page = {
    ...
    loadModal: function(option) {
        var modalHtml = mm.renderHtml(templateAddressModal, option);
    ...
    validateForm: function(addressInfo) {
        ...
        // 验证姓名
        if (mm.validate(addressInfo.receiverName, "notEmpty")) {
            status.isOk = true;
        ...
        // 验证地址
        if (mm.validate(addressInfo.receiverAddress, "notEmpty")) {
            status.isOk = true;
        } else {
            status.isOk = false;
            status.msg = "请填写收件人地址";
        ...
        // 验证号码
        if (mm.validate(addressInfo.receiverPhone, "notEmpty")) {

            if (mm.validate(addressInfo.receiverPhone, "phone")) {
                status.isOk = true;
            } else {
                status.isOk = false;
